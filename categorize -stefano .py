...
def categorize_entire_file(input_csv):
    """
    Carica un CSV, categorizza il 100% dei dati e salva il risultato 
    in un unico file di training.
    """
    try:
    ...
    # --- SALVATAGGIO NELLA CARTELLA DI TRAINING ---
    output_folder = "training_result"
    os.makedirs(output_folder, exist_ok=True)
    
    output_file = os.path.join(output_folder, "training_set_categorized.csv")
    df.to_csv(output_file, index=False, encoding='utf-8')
    
    print(f"\n📄 File di training salvato in: {output_file}")
...
if __name__ == "__main__":
    input_file = "training_result/output_with_text.csv"
    
    print("=" * 80)
    print(f"🚀 AVVIO CATEGORIZZAZIONE COMPLETA DI '{input_file}' 🚀")
    
    result_df = categorize_entire_file(input_file)
